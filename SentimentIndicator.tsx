...
const SentimentIndicator: React.FC<SentimentIndicatorProps> = ({
    ...
    const colorClass =
        SENTIMENT_COLORS[sentiment] || SENTIMENT_COLORS[Sentiment.Neutral];
    const bgColorClass =
        SENTIMENT_BG_COLORS[sentiment] ||
        SENTIMENT_BG_COLORS[Sentiment.Neutral];
        ...
        case Sentiment.Positive:
            icon = (
                <svg
                    ...
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className={`${iconSizeClass} ${colorClass}`}
                >
                    <path
                        strokeLinecap="round"
    ...
    return (
        <div
            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${bgColorClass} ${textSizeClass}`}
        >
            {icon}
