...
import { PredictionData, FullPredictionResponse, GroundingSource, Sentiment, Trend } from '../types';
...
export const getStockSentimentPrediction = async (ticker: string): Promise<FullPredictionResponse> => {
  if (!API_KEY) {
     return {
      ticker: ticker.toUpperCase(),
      ...
      trend: Trend.Unknown,
      confidence_score: 0,
      summary: "API Key is not configured. Please set the API_KEY environment variable.",
      key_factors: [],
      potential_risks: [],
      recent_news_highlights: [],
  ...
  try {
    ...
    const validatedSentiment = Object.values(Sentiment).includes(parsedData.sentiment) ? parsedData.sentiment : Sentiment.Neutral;
    const validatedTrend = Object.values(Trend).includes(parsedData.trend) ? parsedData.trend : Trend.Unknown;
    const validatedKeyFactors = (parsedData.key_factors || []).map(kf => ({
        ...kf,
        impact: Object.values(Sentiment).includes(kf.impact) ? kf.impact : Sentiment.Neutral
    }));
    ...
    console.error(`Error fetching or parsing stock sentiment for ${ticker}:`, error);
    let errorMessage = "Failed to get prediction. The model might be unavailable or returned an unexpected format.";
    if (error instanceof Error) {
        errorMessage = error.message;
        if (error.message.includes("JSON.parse")) {
            errorMessage = "Failed to parse prediction data from the model. The format might be incorrect.";
        } else if (error.message.toLowerCase().includes("api key not valid")) {
            errorMessage = "Invalid API Key. Please check your API_KEY environment variable.";
        }
    }
    ...
    return {
      ...
      trend: Trend.Unknown,
      confidence_score: 0,
      summary: "An error occurred while fetching or processing the prediction.",
      key_factors: [],
      potential_risks: [],
      recent_news_highlights: [],
      error: errorMessage,
...
const extractSources = (chunks: GroundingChunk[] | undefined): GroundingSource[] => {
  if (!chunks) return [];
  return chunks
